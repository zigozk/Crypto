...
def cal_space(ascii_result) :
    ...
    for k in range(int(len(cipherText[10])/2)) :
        ...
        for i in range(11) :
            ...
            cal = 0
            for j in range(11) :
                if ('a' <= ascii_result[i][j][k] <= 'z' or 'A' <= ascii_result[i][j][k] <= 'Z') :
                    cal += 1
            if (cal >= 6) :
                print(k , " " , i , "\n")
                int1 = int(cipherText[i][k * 2 : k * 2 + 2] , 16)
...
def decode(key) :
    for j in range(11) :
        ...
        for i in range(int(len(cipherText[10]) / 2)) :
            int1 = int(cipherText[j][i * 2 : i * 2 + 2] , 16)
...
def correct_key(_plaintext) :
    new_key = ""
    for i in range(int(len(cipherText[10])/2)) :
            int1 = int(cipherText[10][i * 2 : i * 2 + 2] , 16)
            int2 = ord(_plaintext[i])
            new_key += format(int1 ^ int2 , '02x')
        # print(i)
...
_plaintext = "The secret message is: When using a stream cipher, never use the key more than once"
correct_key(_plaintext)
